...
public class FiscalDocumentPdfService {
    ...
    public PdfFile render(FiscalDocument document) {
        if (document == null || document.getTenant() == null) {
            throw new BusinessException("Documento fiscal não encontrado.");
        }
        Long tenantId = document.getTenant().getId();
        TenantFiscalProfile profile = profileRepository.findByTenantId(tenantId).orElse(null);
        ...
        try (PDDocument pdf = new PDDocument(); ByteArrayOutputStream output = new ByteArrayOutputStream()) {
            ...
            try (PDPageContentStream canvas = new PDPageContentStream(pdf, page)) {
                float y = 800;
                y = text(canvas, BOLD, 17, 50, y, document.getStatus().name().equals("DRAFT") ? "COTAÇÃO" : documentTitle(document));
                if (document.getStatus().name().equals("DRAFT")) {
                    y = text(canvas, BOLD, 10, 50, y - 3, "NÃO É DOCUMENTO FISCAL NEM COMPROVATIVO DE PAGAMENTO");
                } else if (document.getStatus().name().equals("CANCELLED")) {
                    y = text(canvas, BOLD, 12, 50, y - 3, "DOCUMENTO CANCELADO");
                }
                y -= 14;
                y = text(canvas, BOLD, 12, 50, y, value(profile != null ? profile.getLegalName() : null, document.getTenant().getNome()));
                y = text(canvas, REGULAR, 10, 50, y, "NIF: " + value(profile != null ? profile.getTaxpayerNumber() : null, document.getTenant().getNif()));
                y = text(canvas, REGULAR, 10, 50, y, joinAddress(profile));
                y -= 10;
                y = text(canvas, REGULAR, 10, 50, y, "Documento: " + value(document.getDocumentNumber(), "—") + "  Série: " + value(document.getSeries(), "—"));
                y = text(canvas, REGULAR, 10, 50, y, "Data: " + (document.getIssuedAt() != null ? DATE_TIME.format(document.getIssuedAt()) : "—"));
                y = text(canvas, REGULAR, 10, 50, y, "Cliente: " + value(document.getCustomerName(), "Consumidor final"));
                y = text(canvas, REGULAR, 10, 50, y, "NIF cliente: " + value(document.getCustomerTaxpayerNumber(), "—"));
                y -= 14;
                ...
                text(canvas, REGULAR, 8, 50, 45, "Documento gerado pelo CONSUMA · valores em AOA");
            }
            pdf.save(output);
            String prefix = document.getStatus().name().equals("DRAFT") ? "cotacao-" : "fatura-";
            return new PdfFile(output.toByteArray(), prefix + safeFilename(document.getDocumentNumber()) + ".pdf");
        } catch (IOException ex) {
            throw new BusinessException("Não foi possível gerar o PDF do documento.");
        }
    }

    private static float text(PDPageContentStream canvas, PDType1Font font, float size, float x, float y, String value) throws IOException {
        canvas.beginText();
    ...
    private static String documentTitle(FiscalDocument document) {
        return switch (document.getDocumentType()) {
            case INTERNAL_INVOICE -> "FATURA";
            case INTERNAL_INVOICE_RECEIPT -> "FATURA-RECIBO";
            case INTERNAL_RECEIPT -> "RECIBO";
            case INTERNAL_CREDIT_NOTE, INTERNAL_CREDIT_NOTE_PLACEHOLDER -> "NOTA DE CRÉDITO";
            case INTERNAL_DEBIT_NOTE -> "NOTA DE DÉBITO";
        };
    }

    private static String joinAddress(TenantFiscalProfile profile) {
        if (profile == null) return "";
        return java.util.stream.Stream.of(profile.getAddress(), profile.getMunicipality(), profile.getProvince(), profile.getCountryCode())
