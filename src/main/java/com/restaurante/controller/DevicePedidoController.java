...
import com.restaurante.service.device.DevicePagamentoService;
...
public class DevicePedidoController {
    ...
    private final DevicePagamentoService devicePagamentoService;
    ...
    public ResponseEntity<ApiResponse<DevicePagamentoResponse>> iniciarPagamento(
            @PathVariable Long pedidoId,
            @RequestHeader(name = "Authorization", required = false) String authorization,
            @RequestHeader(name = "Idempotency-Key", required = false) String idempotencyKey,
            @Valid @RequestBody DeviceIniciarPagamentoRequest request,
            HttpServletRequest http
    ) {
        String ua = http != null ? http.getHeader("User-Agent") : null;
        String ip = http != null ? http.getRemoteAddr() : null;
        DevicePagamentoResponse resp = devicePagamentoService.iniciarPagamento(pedidoId, request, idempotencyKey, ua, ip);
