...
DO $$
BEGIN
    IF to_regclass('tenant_subscriptions') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tenant_subscriptions_tenant') THEN
...
BEGIN
    IF to_regclass('tenant_subscriptions') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tenant_subscriptions_plan') THEN
        ALTER TABLE tenant_subscriptions
...
DO $$
BEGIN
    IF to_regclass('billing_cycles') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_billing_cycles_tenant') THEN
...
BEGIN
    IF to_regclass('billing_cycles') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_billing_cycles_subscription') THEN
        ALTER TABLE billing_cycles
...
BEGIN
    IF to_regclass('usage_events') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_usage_events_tenant') THEN
...
DO $$
BEGIN
    IF to_regclass('usage_adjustments') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_usage_adjustments_tenant') THEN
...
BEGIN
    IF to_regclass('usage_adjustments') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_usage_adjustments_original_event') THEN
        ALTER TABLE usage_adjustments
...
DO $$
BEGIN
    IF to_regclass('usage_aggregations') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_usage_aggregations_tenant') THEN
...
BEGIN
    IF to_regclass('usage_aggregations') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_usage_aggregations_subscription') THEN
        ALTER TABLE usage_aggregations
...
BEGIN
    IF to_regclass('usage_aggregations') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_usage_aggregations_cycle') THEN
        ALTER TABLE usage_aggregations
...
DO $$
BEGIN
    IF to_regclass('tenant_billing_invoice_sequences') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tenant_billing_invoice_sequences_tenant') THEN
...
DO $$
BEGIN
    IF to_regclass('tenant_billing_invoices') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tenant_billing_invoices_tenant') THEN
...
BEGIN
    IF to_regclass('tenant_billing_invoices') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tenant_billing_invoices_subscription') THEN
        ALTER TABLE tenant_billing_invoices
...
BEGIN
    IF to_regclass('tenant_billing_invoices') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tenant_billing_invoices_cycle') THEN
        ALTER TABLE tenant_billing_invoices
...
DO $$
BEGIN
    IF to_regclass('tenant_billing_invoice_lines') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tenant_billing_invoice_lines_invoice') THEN
...
BEGIN
    IF to_regclass('tenant_billing_invoice_lines') IS NOT NULL
        AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_tenant_billing_invoice_lines_tenant') THEN
        ALTER TABLE tenant_billing_invoice_lines
