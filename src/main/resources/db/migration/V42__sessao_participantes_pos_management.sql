...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_sessao_participante_promoted_by_device'
    ) THEN
        ALTER TABLE sessao_consumo_participantes
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_sessao_participante_demoted_by_device'
    ) THEN
        ALTER TABLE sessao_consumo_participantes
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_sessao_participante_removed_by_device'
    ) THEN
        ALTER TABLE sessao_consumo_participantes
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_sessao_participante_blocked_by_device'
    ) THEN
        ALTER TABLE sessao_consumo_participantes
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_sessao_participante_restored_by_device'
    ) THEN
        ALTER TABLE sessao_consumo_participantes
