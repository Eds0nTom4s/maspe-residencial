...
public class CaixaOperadorSessionService {
    ...
    @Transactional
    public CaixaOperadorSession abrirWeb(
        ...
        caixaRepository.findFirstByTenantIdAndOperadorIdAndChannelAndStatusOrderByOpenedAtDesc(
                context.tenantId(), operador.getId(),
                CaixaOperadorSessionChannel.WEB_PDV, CaixaOperadorSessionStatus.OPEN
        ).ifPresent(existing -> {
        ...
        operationalEventLogService.logGeneric(
                ...
                Map.of(
                        ...
                        "turnoId", turno.getId(),
                        "channel", CaixaOperadorSessionChannel.WEB_PDV.name()
                ),
                ip,
                userAgent
        );
    ...
    public CaixaOperadorSession buscarOpenWeb(TenantContext context) {
        ...
        return caixaRepository.findFirstByTenantIdAndOperadorIdAndChannelAndStatusOrderByOpenedAtDesc(
                ...
                CaixaOperadorSessionStatus.OPEN
        ).orElse(null);
    }
    ...
    public CaixaOperadorSession requireOpenWebForPayment(Long tenantId, Long userId, OrdemPagamento ordem) {
        CaixaOperadorSession caixa = caixaRepository
                ...
                .orElse(null);
        if (caixa == null) {
            throw new BusinessException("Caixa web OPEN é obrigatório para confirmar pagamento manual.");
        }
        if (ordem == null
                || ordem.getUnidadeAtendimento() == null
            ...
            throw new BusinessException("O caixa web OPEN não corresponde à unidade e ao turno da ordem.");
        }
        return caixa;
    }
    ...
    public CaixaOperadorSession fechar(DevicePrincipal device,
        ...
        return closeSession(
                caixa, device.tenantId(), closedBy, declaredCashAmount, declaredTpaAmount,
                closeReason, notes, OperationalOrigem.DEVICE_POS,
                "Caixa fechado (operador/device)", ip, userAgent
        );
    }

    @Transactional
    public CaixaOperadorSession fecharWeb(
            TenantContext context,
            Long caixaId,
            BigDecimal declaredCashAmount,
            BigDecimal declaredTpaAmount,
        ...
        User closedBy = userRepository.findById(context.userId())
                .orElseThrow(() -> new BusinessException("Operador não encontrado."));
        return closeSession(
                caixa, context.tenantId(), closedBy, declaredCashAmount, declaredTpaAmount,
                closeReason, notes, OperationalOrigem.TENANT_CASHIER,
                "Caixa fechado (PDV web)", ip, userAgent
        );
    }

    private CaixaOperadorSession closeSession(
            CaixaOperadorSession caixa,
            Long tenantId,
            User closedBy,
            BigDecimal declaredCashAmount,
            BigDecimal declaredTpaAmount,
        ...
        List<OrdemPagamento> ordens = ordemPagamentoRepository.findAllByTenantIdAndCaixaOperadorSessionIdAndStatus(
                tenantId, caixa.getId(), OrdemPagamentoStatus.CONFIRMADA
        );
        ...
        for (OrdemPagamento ordem : ordens) {
            ...
            item.setSource(ordem.getCriadoPorOrigem() != null ? ordem.getCriadoPorOrigem() : origem);
            item.setPagamento(pagamentoGatewayRepository.findByTenantIdAndOrdemPagamentoId(tenantId, ordem.getId()).orElse(null));
            itemRepository.save(item);
        }

        Map<String, Object> metadata = new LinkedHashMap<>();
        metadata.put("caixaId", caixa.getId());
        metadata.put("channel", caixa.getChannel().name());
        metadata.put("instituicaoId", caixa.getInstituicao().getId());
        metadata.put("unidadeAtendimentoId", caixa.getUnidadeAtendimento().getId());
        if (caixa.getDispositivoOperacional() != null) metadata.put("deviceId", caixa.getDispositivoOperacional().getId());
        metadata.put("operadorUserId", caixa.getOperador().getId());
        metadata.put("expectedCashAmount", expectedCash);
        metadata.put("declaredCashAmount", declaredCash);
        metadata.put("cashDifferenceAmount", caixa.getCashDifferenceAmount());
        metadata.put("expectedTpaAmount", expectedTpa);
        metadata.put("declaredTpaAmount", declaredTpa);
        metadata.put("tpaDifferenceAmount", caixa.getTpaDifferenceAmount());
        metadata.put("manualDifferenceAmount", caixa.getManualDifferenceAmount());
        operationalEventLogService.logGeneric(
                OperationalEventType.CAIXA_OPERADOR_SESSION_CLOSED,
                OperationalEntityType.CAIXA_OPERADOR_SESSION,
                caixa.getId(), origem, eventDescription, metadata, ip, userAgent
        );
