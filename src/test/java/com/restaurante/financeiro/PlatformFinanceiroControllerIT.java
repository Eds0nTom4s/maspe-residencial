...
class PlatformFinanceiroControllerIT extends PostgresTestcontainersConfig {
    ...
    void platformAdmin_canListGlobalPayments_andSeeCallbacksWithoutTenant() throws Exception {
        ...
        semTenant.setExternalReference("UNKNOWN");
        ...
        assertThat(cbRoot.at("/data/content").size()).isEqualTo(1);
        assertThat(cbRoot.at("/data/content/0/externalReference").asText()).isEqualTo("UNKNOWN");

        // detalhe do callback (platform)
        Long logId = callbackLogRepository.findAll().stream()
                .filter(l -> "UNKNOWN".equals(l.getExternalReference()))
        ...
        assertThat(dRoot.at("/data/externalReference").asText()).isEqualTo("UNKNOWN");
    }
